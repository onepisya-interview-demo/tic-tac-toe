...
- **LSP 服务端工具（typescript-language-server / yaml-language-server / bash-language-server）走 vp 全局安装，禁止作为项目 npm 依赖** —— 它们是 Codex `lsp.*` MCP 的语言服务端，永不进 Next.js runtime bundle。vp smart-shim 把每个 package 装到独立 hash 目录，`typescript-language-server` 因此需用 `.codex/lsp-client.json` 的 `initialization.tsserver.path` 指向 vp 装的 `bin/tsserver` 脚本（path 包含 install hash，重装后需同步更新）；bash / yaml 无需此 workaround。任何 LSP 改动必须：(1) 三个 server 通过 `vp add -g` / `vp rm -g` 同步增删；(2) `.codex/lsp-client.json` 同步；(3) `pnpm vitest run` + `pnpm typecheck` 确认不破 Gauntlet。详见 `.omo/plans/lsp-revert-to-global.md`。
