...
| L2 状态集成 | tests/store/store.test.ts | vitest + `lib/game-net` mock | Zustand 状态机与 `lib/game-net.ts` 4 函数契约（postSession / fetchPlayerStats / postMerge / postOutcome）+ 网络失败回退 + AbortController 8s timeout |
| L3 数据层 | tests/db/db.test.ts | vitest + @libsql/client | 真本地 sqlite file: 往返 + spy http(s) 分支 + legacy schema reconcile 迁移测试（W1 `name TEXT UNIQUE` 加列）；每测试 tmpdir 隔离 + 模块 reset |
