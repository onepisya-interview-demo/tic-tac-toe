...
/**
 * RoomGateMount (ulw-room-migration-home-landing W2 D-2):
 *
 * Home-page mount effect that listens for `ttt:room-required` window
 * events (dispatched by StartGameButton when the user clicks the
 * online CTA without a name) and opens the RoomGateDialog. The
 * event detail carries `{ mode, href }` so the dialog's onConfirm
 * knows where to navigate and which mode to start.
 *
 * Pattern mirrors HomeDialogMount: a single home-page mount, an
 * event subscription, and zero ambient network calls (D-1 contract:
 * home page = hero + guidance + CTAs + static entry; nothing else
 * auto-fires on mount).
 *
 * Race-safety: the dialog's localStorage write happens AFTER the
 * server returns success, so a transient network failure leaves the
 * user without a name (the dialog shows an inline error and keeps
 * the typed value for retry — R4/R6/R9 branches).
 */
