...
// a POST cannot sneak through a value the GET would reject). Trim, then
...
// W3 (ulw-name-login-one-truth): the legacy "PUT { name }" idempotent
// empty-row bootstrap (ulw-solo-sync-rebuild.md B-T1) has been
// retired. Registration / login now flow exclusively through POST
// /api/player-session (which returns { stats, existed } so the client
// can branch on 注册 vs 登录). All write paths into a name row are
