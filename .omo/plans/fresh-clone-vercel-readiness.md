...
- [x] 2. README.md Quick-start-for-newcomers section + 适用 blockquote at top
  ...
  References (executor has NO interview context - be exhaustive): /private/tmp/ulw-demo/README.md:1-30 (current top-of-file structure to preserve); /private/tmp/ulw-demo/README.md:42-51 (existing §本地开发 to keep aligned); /private/tmp/ulw-demo/docs/local-turso-setup.md:5 (the 适用 blockquote format to mirror — node v24.20.0, pnpm 10.10.0, turso CLI v1.0.32 confirmed by user on 2026-09-08, all satisfy the ≥ bounds)
  Acceptance criteria (agent-executable): `grep -c '^## Quick start' /private/tmp/ulw-demo/README.md` returns 1; `awk '/^## Quick start/,/^## /' /private/tmp/ulw-demo/README.md | grep -c 'env.local'` >= 1; `grep -F '适用：本仓库 main 分支、Node.js ≥ 20、pnpm ≥ 10、Turso CLI ≥ 0.100' /private/tmp/ulw-demo/README.md` >= 1 (the 适用 blockquote is the first content after the title, byte-identical to docs/local-turso-setup.md:5).
...
- [x] 3. README.md §本地开发 add explicit "no .env.local needed" sentence
  ...
  Parallelization: Wave 2 | Blocked by: 1 | Blocks: 10
  References (executor has NO interview context - be exhaustive): /private/tmp/ulw-demo/README.md:42-51 (target section); /private/tmp/ulw-demo/lib/db.ts:47-49 (the unset->file: branch being documented)
  Acceptance criteria (agent-executable): `awk '/^## 本地开发/,/^## /' /private/tmp/ulw-demo/README.md | grep -c '不需要建'` >= 1.
...
- [x] 4. README.md §部署 restructure into 4 subsections
  ...
  Parallelization: Wave 2 | Blocked by: 1 | Blocks: 10
  References (executor has NO interview context - be exhaustive): /private/tmp/ulw-demo/README.md:124-144 (current §部署 to replace); /private/tmp/ulw-demo/lib/db.ts:47-65 (URL scheme contract being documented); /private/tmp/ulw-demo/.env.example (env var table source of truth)
  Acceptance criteria (agent-executable): `grep -c '^### Vercel + Turso' /private/tmp/ulw-demo/README.md` returns 1; `grep -c '^### Vercel 排错' /private/tmp/ulw-demo/README.md` returns 1; `grep -c '<your-db-name>' /private/tmp/ulw-demo/README.md` >= 1; `grep -c 'tic-tac-toe-onepisya' /private/tmp/ulw-demo/README.md` returns 0 (public repo uses placeholders).
...
- [x] 5. .env.example rewrite top comment to make unset->file: explicit
  ...
  Parallelization: Wave 2 | Blocked by: 1 | Blocks: 10
  References (executor has NO interview context - be exhaustive): /private/tmp/ulw-demo/.env.example (current file); /private/tmp/ulw-demo/lib/db.ts:47-49 (the fallback being documented)
  Acceptance criteria (agent-executable): `grep -c '不需要' /private/tmp/ulw-demo/.env.example` >= 1; `grep -c '^DATABASE_URL=file:' /private/tmp/ulw-demo/.env.example` returns 1 (placeholder preserved); `grep -c '^# DATABASE_AUTH_TOKEN' /private/tmp/ulw-demo/.env.example` returns 1.
...
- [x] 6. CONTRIBUTING.md Quick-start update
  ...
  Parallelization: Wave 2 | Blocked by: 1 | Blocks: 10
  References (executor has NO interview context - be exhaustive): /private/tmp/ulw-demo/CONTRIBUTING.md:8-19 (target section); /private/tmp/ulw-demo/docs/operations.md (link target)
  Acceptance criteria (agent-executable): `grep -c '无需配置' /private/tmp/ulw-demo/CONTRIBUTING.md` >= 1; `grep -c 'docs/operations.md#环境变量' /private/tmp/ulw-demo/CONTRIBUTING.md` returns 1.
...
- [x] 7. docs/operations.md §部署 alignment + §环境变量 适用 triplet
  ...
  Parallelization: Wave 2 | Blocked by: 1 | Blocks: 10
  References (executor has NO interview context - be exhaustive): /private/tmp/ulw-demo/docs/operations.md:7 (the §环境变量 prerequisite list to augment); /private/tmp/ulw-demo/docs/operations.md:50-58 (target §部署 section); /private/tmp/ulw-demo/README.md §部署 (link target); /private/tmp/ulw-demo/docs/local-turso-setup.md:5 (reference 适用 blockquote format)
  Acceptance criteria (agent-executable): `awk '/^## 部署/,/^## /' /private/tmp/ulw-demo/docs/operations.md | grep -c 'README.md#部署'` >= 1; `awk '/^## 部署/,/^## /' /private/tmp/ulw-demo/docs/operations.md | grep -c 'DATABASE_URL'` >= 1; `grep -c 'pnpm ≥ 10' /private/tmp/ulw-demo/docs/operations.md` >= 1; `grep -c 'Turso CLI ≥ 0.100' /private/tmp/ulw-demo/docs/operations.md` >= 1; the original `Node.js ≥ 20` line is preserved (`grep -c 'Node.js ≥ 20' /private/tmp/ulw-demo/docs/operations.md` >= 1).
...
- [x] 8. docs/local-turso-setup.md cross-link + 适用 blockquote preserved
  ...
  Parallelization: Wave 2 | Blocked by: 1 | Blocks: 10
  References (executor has NO interview context - be exhaustive): /private/tmp/ulw-demo/docs/local-turso-setup.md:1-3 (insertion point); /private/tmp/ulw-demo/docs/local-turso-setup.md:5 (适用 blockquote source of truth, byte-identical preservation required); /private/tmp/ulw-demo/README.md §部署 (link target)
  Acceptance criteria (agent-executable): `head -10 /private/tmp/ulw-demo/docs/local-turso-setup.md | grep -c 'README.md#部署'` >= 1; the original "Vercel 部署走的是同一条线路但配环境变量的地方不同" line is still present (`grep -c 'Vercel 部署走的是同一条线路' /private/tmp/ulw-demo/docs/local-turso-setup.md` returns 1); the 适用 blockquote is byte-identical to its pre-edit form (`grep -F '适用：本仓库 main 分支、Node.js ≥ 20、pnpm ≥ 10、Turso CLI ≥ 0.100。' /private/tmp/ulw-demo/docs/local-turso-setup.md` returns 1, AND `git diff HEAD -- docs/local-turso-setup.md | grep -c '^[+-]'` for line 5 only shows the cross-link insertion, NOT a 适用 rephrasing).
